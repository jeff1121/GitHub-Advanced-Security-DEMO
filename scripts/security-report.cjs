...
async function buildReport(client, pr) {
  ...
  const lines = [REPORT_MARKER, '## Security findings and remediation evidence', '',
    ...
    'Explicit workflow_dispatch verification runs are preferred when present. Native PR-triggered runs may separately require human approval; this report does not waive that requirement.',
    ...((runsResponse.workflow_runs || []).some((item) => item.head_sha === sha && item.conclusion === 'action_required')
      ? ['**Human workflow approval is pending on at least one PR-triggered run.** Review the code before approving those runs.'] : []),
    ...['CI', 'CodeQL'].map((name) => {
